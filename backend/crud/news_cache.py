...
async def get_categories(db:AsyncSession, skip: int = 0, limit: int = 100):
    #先尝试从缓存中获取数据
    cache_categories = await get_cached_category()
    if cache_categories:
    ...
    if categories:
        ...
        await set_cached_category(categories)
    # 返回数据
    return categories

# async def get_list(db:AsyncSession,category_id:int=0,page:int=1,page_size:int=10):
#     # 先尝试从缓存中获取数据
#     cache_news_list = await get_cached_news_list(category_id,page,page_size)
#     if cache_news_list:
...
#     # 写入缓存
#     if news_list:
#         news_list = [{NewsItemBase.model_dump(item).} for item in news_list]
